    ...
    double precision :: a,b,corfac,delta,w
    double precision :: a0i,a0r,a1i,a1r,a2i,a2r,a3i,a3r,arg,c,s, &
    t,t2,t4,t6
    double precision :: cth,ctth,spth2,sth,sth4i,stth,th,th2,th4,tmth2, &
    tth4i
    complex*16 cl,sl,cr,sr,corre,corim,endpts(8)
    th=w*delta
!      write(*,*) "theta=",th,"w=",w,"delta=",delta
    if (a >= b .OR. th < 0.d0 .OR. th > 3.1416d0)pause &
    'bad arguments to dftcor'
    if(abs(th) < 5.d-2)then
        t=th
        t2=t*t
        t4=t2*t2
        t6=t4*t2
